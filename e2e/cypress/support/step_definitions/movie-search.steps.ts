...
Given(/^that the user is in the "(Home|Movie Search)" page$/, (option: string)=> {
    baseActions.generateTokenAndAuthentication();
    baseActions.goToThePage(option);
});
Given(/^the user clicks on the "(Search|Add to Favorites|Remove from Favorites)" button$/, (option: string)=> {
